...
export default function SearchBar({ tourneyid }: { tourneyid: number }) {
    ...
    (async () => {
      ...
      if (debouncedQuery.length > 0 && debouncedQuery !== ".") {
        const response = await fetch(
          `${process.env.PUBLIC_URL}/api/tourneys/${tourneyid}/teamsearch/${debouncedQuery}`
        );
        ...
        {filteredSuggestions.length > 0 && (
          ...
          >
            {suggestions.map((suggestions: any) => (
              <Link
                key={suggestions.id}
                href={`/admin/tourneys/${tourneyid}/team/${suggestions.teamid}`}
              >
                <Combobox.Option
                  key={suggestions.id}
                  value={suggestions}
        ...
        {query !== "" && filteredSuggestions.length === 0 && (
          <div className="py-14 px-4 text-center sm:px-14">
            ...
            <p className="mt-4 text-sm text-gray-900">
              No tournament found with that searchterm.
            </p>
          </div>
        )}
      </Combobox>
